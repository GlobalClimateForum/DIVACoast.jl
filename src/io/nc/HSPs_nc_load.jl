...
function load_hsps_nc(::Type{IT}, ::Type{DT}, filename::String)::Dict{IT,HypsometricProfile{DT}} where {IT<:Integer,DT<:Real}
  ...
  for i in 1:size(ids, 1)
    ...
    hpsf = HypsometricProfile(width[i], width_unit, copy(el), el_unit, area, area_unit, s_exposure, static_exp_names, static_exp_units, d_exposure, dynamic_exp_names, dynamic_exp_units)
