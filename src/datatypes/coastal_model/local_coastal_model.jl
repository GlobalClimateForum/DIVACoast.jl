...
#  edam_area = quadgk(x -> (damage_bathtub_standard_ddf(lcm.coastal_plain_model, x, hdd_area, :area) * pdf(lcm.surge_model, x)), lcm.coastal_plain_model.elevation[1], maximum(lcm.surge_model), rtol=1e-3)[1]
  edam_area = expected_damage_integral_computation(lcm, "area", x -> (damage_bathtub_standard_ddf(lcm.coastal_plain_model, x, hdd_area, :area) * pdf(lcm.surge_model, x)), lcm.coastal_plain_model.elevation[1], maximum(lcm.surge_model), 1e-3)
  ...
  for ind in 1:size(lcm.coastal_plain_model.cummulativeDynamicExposure, 2)
    edam_dynamic[ind] = expected_damage_integral_computation(lcm, String(lcm.coastal_plain_model.dynamicExposureSymbols[ind]), x -> (damage_bathtub_standard_ddf(lcm.coastal_plain_model, x, hdds_dynamic[ind], lcm.coastal_plain_model.dynamicExposureSymbols[ind]) * pdf(lcm.surge_model, x)), lcm.coastal_plain_model.elevation[1], maximum(lcm.surge_model), 1e-3)
  end
...
function expected_damage_integral_computation(lcm::LocalCoastalImpactModel{DT, DATA}, s::String, f, lower, upper, tolerance) where {DT<:Real, DATA}
  try
    quadgk(f, lower, upper, rtol=tolerance)[1]
#      x -> (damage_bathtub(lcm.coastal_plain_model, convert(DT, x), ddf, s) * pdf(lcm.surge_model, x)), lcm.coastal_plain_model.elevation[1], maximum(lcm.surge_model), rtol=1e-3)[1]
  catch
