...
function expected_damage_standard_ddf(lcm::LocalCoastalModel{DT}, hdd_area::DT, hdds_static::Array{DT}, hdds_dynamic::Array{DT}) where {DT<:Real}
  edam_area = quadgk(x -> (damage_standard_ddf(lcm.coastal_plain_model, :area, x, hdd_area) * pdf(lcm.surge_model, x)), 0, maximum(lcm.surge_model))[1]
  ...
  for ind in 1:size(lcm.coastal_plain_model.cummulativeStaticExposure, 2)
    edam_static[ind] = quadgk(x -> (damage_standard_ddf(lcm.coastal_plain_model, lcm.coastal_plain_model.staticExposureSymbols[ind], x, hdds_static[ind]) * pdf(lcm.surge_model, x)), minimum(0), maximum(lcm.surge_model))[1]
  end

  for ind in 1:size(lcm.coastal_plain_model.cummulativeDynamicExposure, 2)
    edam_dynamic[ind] = quadgk(x -> (damage_standard_ddf(lcm.coastal_plain_model, lcm.coastal_plain_model.dynamicExposureSymbols[ind], x, hdds_dynamic[ind]) * pdf(lcm.surge_model, x)), minimum(0), maximum(lcm.surge_model))[1]
  end
...
function expected_damage_standard_ddf(lcm::LocalCoastalModel, s::Symbol, hdd::Real)
  quadgk(x -> (damage_standard_ddf(lcm.coastal_plain_model, s, x, hdd) * pdf(lcm.surge_model, x)), minimum(0), maximum(lcm.surge_model))[1]
end
