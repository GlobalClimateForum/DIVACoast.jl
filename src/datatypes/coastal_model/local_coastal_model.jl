...
function expected_damage_bathtub_standard_ddf(lcm::LocalCoastalImpactModel{DT, DATA}, hdd_area::DT, hdds_static::Array{DT}, hdds_dynamic::Array{DT}, tol::Real=1e-3) where {DT<:Real, DATA}
#  edam_area = quadgk(x -> (damage_bathtub_standard_ddf(lcm.coastal_plain_model, x, hdd_area, :area) * pdf(lcm.surge_model, x)), lcm.coastal_plain_model.elevation[1], maximum(lcm.surge_model), rtol=1e-3)[1]
  ...
  edam_area = expected_damage_integral_computation(lcm, "area", x -> (damage_bathtub_standard_ddf(lcm.coastal_plain_model, x, hdd_area, :area) * pdf(lcm.surge_model, x)), lower_limit, maximum(lcm.surge_model), tol)
...
expected_damage_bathtub_standard_ddf(lcm::LocalCoastalImpactModel{DT,DATA}, hdd_area::Real, hdds_static, hdds_dynamic, tol::Real=1e-3) where {DT<:Real, DATA} = 
  if (hdds_static == []) && (hdds_dynamic == [])
    expected_damage_bathtub_standard_ddf(lcm, convert(DT, hdd_area), Matrix{DT}(undef, 0, 0), Matrix{DT}(undef, 0, 0), tol)
  elseif (hdds_static == [])
    expected_damage_bathtub_standard_ddf(lcm, convert(DT, hdd_area), Matrix{DT}(undef, 0, 0), convert(Array{DT}, hdds_dynamic), tol)
  elseif (hdds_dynamic == [])
    expected_damage_bathtub_standard_ddf(lcm, convert(DT, hdd_area), convert(Array{DT}, hdds_static), Matrix{DT}(undef, 0, 0), tol)
  else
    expected_damage_bathtub_standard_ddf(lcm, convert(DT, hdd_area), convert(Array{DT}, hdds_static), convert(Array{DT}, hdds_dynamic), tol)
  end
...
function expected_damage_bathtub(lcm::LocalCoastalImpactModel{DT}, ddf_area::Function, ddf_static::Array{Function}, ddf_dynamic::Array{Function}, tol::Real=1e-3) where {DT<:Real}
  lower_limit = if lcm.protection_level==0 lcm.coastal_plain_model.elevation[1] else quantile(lcm.surge_model,1-1/lcm.protection_level) end
  ...
  for ind in 1:size(lcm.coastal_plain_model.cummulativeStaticExposure, 2)
    edam_static[ind] = expected_damage_integral_computation(lcm, String(lcm.coastal_plain_model.staticExposureSymbols[ind]), x -> damage_bathtub(lcm.coastal_plain_model, x, ddf_static[ind], lcm.coastal_plain_model.staticExposureSymbols[ind]) * pdf(lcm.surge_model, x), lower_limit, maximum(lcm.surge_model), tol)
  end

  for ind in 1:size(lcm.coastal_plain_model.cummulativeDynamicExposure, 2)
    edam_dynamic[ind] = expected_damage_integral_computation(lcm, String(lcm.coastal_plain_model.dynamicExposureSymbols[ind]), x -> damage_bathtub(lcm.coastal_plain_model, x, ddf_dynamic[ind], lcm.coastal_plain_model.dynamicExposureSymbols[ind]) * pdf(lcm.surge_model, x), lower_limit, maximum(lcm.surge_model), tol)
  end
...
damage_bathtub_standard_ddf(lcm::LocalCoastalImpactModel{DT, DATA}, wl, hdd_area, hdds_static, hdds_dynamic, tol::Real=1e-3) where {DT<:Real,DATA} = damage_standard_ddf(lcm.coastal_plain_model, wl, hdd_area, hdds_static, hdds_dynamic, tol)
damage_bathtub_standard_ddf(lcm::LocalCoastalImpactModel{DT, DATA}, wl::T1, hdd::T2, s::Symbol, tol::Real=1e-3) where {DT<:Real,T1<:Real,T2<:Real,DATA} = damage_standard_ddf(lcm.coastal_plain_model, s, convert(DT, wl), convert(DT, hdd), tol)
