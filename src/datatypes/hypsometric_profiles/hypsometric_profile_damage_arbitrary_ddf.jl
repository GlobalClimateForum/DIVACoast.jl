  ...
  for ind in 1:size(hspf.elevation, 1)-1
    if hspf.elevation[ind] > wl
      ...
      if (Δ_area != 0)
        ...
        dam_t = partial_damage(hspf, wl, ddf_area, ddfs_static, ddfs_dynamic, sl, wl_low, wl_high, ρ_area, ρ_exp_st, ρ_exp_dy)
...
function damage(hspf::HypsometricProfile{DT}, s::Symbol, wl::DT, ddf::Function)::DT where {DT<:Real}
  ...
  for ind in 1:size(hspf.elevation, 1)-1
    if hspf.elevation[ind] > wl
      ...
      if Δ_area != 0
        ...
        factor = quadgk(x -> ddf(wl - x), wl_low, wl_high)[1]
        dam += factor * ρ_exp / sl
      end
    end
  end
...
damage(hspf::HypsometricProfile{DT}, wl::Real, ddf_area::Function, ddfs_static::Vector{Any}, ddfs_dynamic::Vector{Any}) where {DT<:Real} =
 ...
 damage_standard_ddf(hspf::HypsometricProfile{DT}, s::Symbol, wl::T, ddf::Function) where {DT<:Real,T<:Real} = damage_standard_ddf(hspf, s, convert(DT, wl), ddf)
