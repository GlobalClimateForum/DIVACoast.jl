...
# todo: handle existing files. 
function save_data_complete_csv(sgr::SparseGeoArray{DT,IT}, filename::String, lonlat::Bool=false) where {DT<:Real,IT<:Integer}
  fn = split(filename, ".")
  if (size(fn) == 1)
    return
  end
  ...
  open(filename, "w") do f
    ...
    write(f, "\n")
    write(f, "$(sgr.projref)\n")
    write(f, "$(sgr.xsize),$(sgr.ysize)\n")
    write(f, "$(sgr.nodatavalue)\n")
    write(f, "$(convert(Int8,lonlat))\n")

    for (indices, data) in sgr.data
      if lonlat
...
function private_insert_data!(sgr::SparseGeoArray{DT,IT}, data::Vector{Float32}, ysize::Integer, r_offset::Integer) where {DT<:Real,IT<:Integer}
  for j in 1:sgr.xsize
    for i in 1:ysize
      ...
      if val != sgr.nodatavalue
        sgr.data[(j, i + r_offset)] = val
      end
    end
  end
end
