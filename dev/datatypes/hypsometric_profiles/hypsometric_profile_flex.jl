...
mutable struct HypsometricProfile{DT <: Real}
  ...
  # Constructors
  function HypsometricProfile(w::DT, elevations::Array{DT}, area::Array{DT}, s_exposure :: StructArray{T1}, d_exposure :: StructArray{T2}, logger :: ExtendedLogger = ExtendedLogger()) where {T1,T2,DT <: Real}
 ...
 function HypsometricProfile(w::DT, elevations::Array{DT}, area::Array{DT}, s_exposure :: Array{DT,2}, d_exposure :: Array{DT,2}, logger :: ExtendedLogger = ExtendedLogger()) where {DT <: Real}
    # String(nameof(var"#self#"))
    if (length(elevations)!=length(area))  logg(logger,Logging.Error,@__FILE__,"","\n length(elevations) != length(area) as length($elevations) != length($area) as $(length(elevations)) != $(length(area))") end
    if ((size(s_exposure,1)>0) && (length(elevations)!=size(s_exposure,1))) logg(logger,Logging.Error,@__FILE__,"","\n length(elevations) != size(s_exposure,1) as length($elevations) != size($s_exposure,1) as $(length(elevations)) != $(size(s_exposure,1))") end
    if ((size(d_exposure,1)>0) && (length(elevations)!=size(d_exposure,1)))  logg(logger,Logging.Error,@__FILE__,"","\n length(elevations) != size(d_exposure,1)  as length($elevations) != size($d_exposure,1)  as $(length(elevations)) != $(size(d_exposure,1))") end
    ...
    new{DT}(w, elevations, cumsum(area), cumsum(s_exposure,dims=1), cumsum(d_exposure,dims=1), ntuple(i ->  Symbol("s_exposure_name_$i"), size(s_exposure, 2)), ntuple(i ->  Symbol("d_exposure_name_$i"), size(d_exposure, 2)), logger)
  end


 function HypsometricProfile(w::DT, elevations::Vector{DT}, area::Vector{DT}, s_exposure :: Array{DT,2}, d_exposure :: Array{DT,2}, s_exposure_names :: Array{String}, d_exposure_names :: Array{String}, logger :: ExtendedLogger = ExtendedLogger()) where {DT <: Real}
    # String(nameof(var"#self#"))
    if (length(elevations)!=length(area))  logg(logger,Logging.Error,@__FILE__,"","\n length(elevations) != length(area) as length($elevations) != length($area) as $(length(elevations)) != $(length(area))") end
    if ((size(s_exposure,1)>0) && (length(elevations)!=size(s_exposure,1))) logg(logger,Logging.Error,@__FILE__,"","\n length(elevations) != size(s_exposure,1) as length($elevations) != size($s_exposure,1) as $(length(elevations)) != $(size(s_exposure,1))") end
    if ((size(d_exposure,1)>0) && (length(elevations)!=size(d_exposure,1))) logg(logger,Logging.Error,@__FILE__,"","\n length(elevations) != size(d_exposure,1)  as length($elevations) != size($d_exposure,1)  as $(length(elevations)) != $(size(d_exposure,1))") end
    ...
    new{DT}(w, elevations, cumsum(area), cumsum(s_exposure,dims=1), cumsum(d_exposure,dims=1), Tuple(map(x -> Symbol(x),s_exposure_names)), Tuple(map(x -> Symbol(x),d_exposure_names)), logger)
  end
